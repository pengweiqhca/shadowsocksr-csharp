      ...
      private const String PLATFORM_DEFAULT_ENCODING = "UTF-8";
        ...
        public static string guessEncoding(byte[] bytes, IDictionary<DecodeHintType, object> hints)
        {
                 ...
                 i++)
            {
                ...
                if (canBeISO88591)
                {
                    if (value is > 0x7F and < 0xA0)
                    {
                        canBeISO88591 = false;
                    }
                    else if (value is > 0x9F and (< 0xC0 or 0xD7 or 0xF7))
                    {
                        isoHighOther++;
                    } //else {
                    //isoHighChars++;
                ...
                if (canBeShiftJIS)
                {
                    if (sjisBytesLeft > 0)
                    {
                        if (value is < 0x40 or 0x7F or > 0xFC)
                        {
                            canBeShiftJIS = false;
                        }
                        else
                    ...
                    else if (value is 0x80 or 0xA0 or > 0xEF)
                    {
                        canBeShiftJIS = false;
                    }
                    else if (value is > 0xA0 and < 0xE0)
                    {
                        sjisKatakanaChars++;
