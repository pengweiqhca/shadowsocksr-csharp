...
{
    private static readonly ResultPoint[] NO_POINTS = Array.Empty<ResultPoint>();
    ...
    /// </summary>
    /// <returns></returns>
    protected Decoder getDecoder() => decoder;
    ...
    private static BitMatrix extractPureBits(BitMatrix image)
    {
        ...
        var matrixWidth = (int)Math.Round((right - left + 1) / moduleSize);
        var matrixHeight = (int)Math.Round((bottom - top + 1) / moduleSize);
        ...
        var nudge = (int)(moduleSize / 2.0f);
        top += nudge;
        left += nudge;

        // But careful that this does not sample off the edge
        var nudgedTooFarRight = left + (int)((matrixWidth - 1) * moduleSize) - (right - 1);
        ...
        var nudgedTooFarDown = top + (int)((matrixHeight - 1) * moduleSize) - (bottom - 1);
        ...
        {
            var iOffset = top + (int)(y * moduleSize);
