...
internal class AuthChain_a : VerifySimpleBase
{
    ...
    protected int GetRandLen(int datalength) => GetRandLen(datalength, random_client, last_client_hash);
    ...
    public void PackAuthData(byte[] data, int datalength, byte[] outdata, out int outlength)
    {
        ...
        encryptor = EncryptorFactory.GetEncryptor("rc4", Convert.ToBase64String(user_key) + Convert.ToBase64String(last_client_hash, 0, 16), false);
    ...
    public override byte[] ClientUdpPreEncrypt(byte[] plaindata, int datalength, out int outlength)
    {
        ...
        encryptor = EncryptorFactory.GetEncryptor("rc4", Convert.ToBase64String(user_key) + Convert.ToBase64String(md5data, 0, 16), false);
...
internal class AuthChain_b : AuthChain_a
{
    ...
    {
        var random = new xorshift128plus();
...
internal class AuthChain_c : AuthChain_b
{
    ...
    protected override void InitDataSizeList()
    {
        var random = new xorshift128plus();
        random.init_from_bin(Server.key);
        var len = (int)(random.next() % (8 + 16) + (4 + 8));
        var data_list = new List<int>();
        for (var i = 0; i < len; ++i)
        {
            data_list.Add((int)(random.next() % 2340 % 2040 % 1440));
    ...
    protected override int GetRandLen(int datalength, xorshift128plus random, byte[] last_hash)
    {
        ...
        var final_pos = pos + (int)(random.next() % (ulong)(data_size_list0.Length - pos));
...
internal class AuthChain_d : AuthChain_c
{
    ...
    {
        if (data_list[^1] < 1300 && data_list.Count < 64)
        {
            data_list.Add((int)(random.next() % 2340 % 2040 % 1440));
...
internal class AuthChain_e : AuthChain_d
{
...
internal class AuthChain_f : AuthChain_e
{
