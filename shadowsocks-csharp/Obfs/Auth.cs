...
namespace Shadowsocks.Obfs
{
    ...
    public class AuthSHA1 : VerifySimpleBase
    {
        ...
        public override object InitData() => new AuthData();
        ...
        public void PackAuthData(byte[] data, int datalength, byte[] outdata, out int outlength)
        {
            ...
            var crc32 = Util.CRC32.CalcCRC32(Server.key, (int)Server.key.Length);
    ...
    public class AuthSHA1V2 : VerifySimpleBase
    {
        ...
        public void PackAuthData(byte[] data, int datalength, byte[] outdata, out int outlength)
        {
            ...
            var crc32 = Util.CRC32.CalcCRC32(crcdata, (int)crcdata.Length);
            BitConverter.GetBytes((uint)crc32).CopyTo(outdata, 0);
    ...
    public class AuthAES128SHA1 : VerifySimpleBase
    {
        ...
        public override object InitData() => new AuthDataAes128();
...

            {
                ...
                if (index_of_split > 0)
                {
                    try
                    {
                        ...
                        user_key = hash(System.Text.Encoding.UTF8.GetBytes(Server.param[(index_of_split + 1)..]));
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                {
                    if (Server.data is AuthDataAes128 { tree: { } } authData)
                    {
                        lock (authData)
                        {
                            authData.tree.Update(packet_cnt);
