...
namespace Shadowsocks.Obfs
{
    class HttpSimpleObfs : ObfsBase
    {
        ...
        private static readonly string[] _request_path = new[]
        {
            "", "",
            "login.php?redir=", "",
            "register.php?code=", "",
        ...
        private static readonly string[] _request_useragent = new[]
        {
        ...
        private static readonly int _useragent_index = new Random().Next(_request_useragent.Length);

        private bool has_sent_header;
        //private bool has_recv_header;
        private bool raw_trans_sent;
        private bool raw_trans_recv;
        private readonly List<byte[]> data_buffer = new();
    ...
    class TlsTicketAuthObfs : ObfsBase
    {
        ...
        private readonly List<byte[]> data_sent_buffer = new();
        private byte[] data_recv_buffer = Array.Empty<byte>();
        private uint send_id;
        private readonly bool fastauth;
        ...
        public override object InitData() => new TlsAuthData();
        ...
        public override byte[] ClientEncode(byte[] encryptdata, int datalength, out int outlength)
        {
            ...
            if ((handshake_status & 3) != 0)
            {
                ...
                {
                    var finish_len_set = new[] { 32 }; //, 40, 64
                    ...
                    handshake_finish[^1] = (byte)finish_len;
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(host) && host[^1] >= '0' && host[^1] <= '9' && Server.param.Length == 0)
                {
                    host = "";
                }
        ...
        public override byte[] ClientDecode(byte[] encryptdata, int datalength, out int outlength, out bool needsendback)
        {
            ...
            else
            {
                ...
                outlength = 0;
                needsendback = false;
                var outdata = new byte[data_recv_buffer.Length];
